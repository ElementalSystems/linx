    ...
    bot: function(el,lk) {
      ...
      switch (b) {
        ...
        case 2: bot.lineWidth(0.5).fillStyle("hsla("+mods(s2,s2v)+",50%,"+l+"%,.5)").circle(.3,0,rdm(.03,.04),true);
                ...
                break;
        case 3: bot=bot.lineStyle("hsla("+mods(s2,s2v)+",100%,"+l+",.2)").lineWidth(rdm(1.5,3.5))
...
function qThm(id,c1,c2,l,bk)
{
  switch (id) {
    ...
    case 8: theme(3,3,c1,10,c2,30,10,0,30,15,.5,l); break;
...
function thm(fin,bk) {
  qThm(Number(fin.charAt(3)),Number(fin.substring(4,6))*10,Number(fin.substring(6,8))*10,Number(fin.charAt(8))*10,bk);
}
